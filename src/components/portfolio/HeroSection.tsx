        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-16 items-center">
          ...
          <div className="space-y-6 sm:space-y-8 lg:space-y-10 text-center lg:text-left order-2 lg:order-1">
            <header className="space-y-3 sm:space-y-6">
              <p className="text-blue-200 text-sm sm:text-lg font-medium tracking-wide" role="doc-subtitle">Hi, I'm</p>
              <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-bold leading-[0.9] tracking-tight">
                <span className="text-white">Sagar Singh</span>
              </h1>
              <h2 className="text-base sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-semibold text-blue-100 px-2 sm:px-0">
                Creative Mind behind many <span className="text-blue-400 font-bold">Viral Contents.</span>
              </h2>
            </header>

            {/* Social Stats - Mobile Optimized */}
            <nav className="flex flex-col gap-2 sm:gap-4 justify-center lg:justify-start px-2 sm:px-0" role="navigation" aria-label="Social Media Profiles">
              <a 
                href="https://www.instagram.com/sagar.edits/" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg sm:rounded-2xl p-3 sm:p-6 flex items-center space-x-3 sm:space-x-4 group cursor-pointer hover:bg-white/15 transition-all duration-300 touch-manipulation min-h-[50px] sm:min-h-[60px]"
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 lg:gap-5 justify-center lg:justify-start px-2 sm:px-0" role="group" aria-label="Call to Action Buttons">
              <Button 
                onClick={scrollToPortfolio} 
                className="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 sm:px-8 lg:px-10 py-3 sm:py-6 lg:py-7 text-sm sm:text-lg rounded-lg sm:rounded-2xl shadow-lg transition-all duration-300 touch-manipulation min-h-[44px] sm:min-h-[52px] w-full sm:w-auto"
                aria-label="View Sagar Singh's portfolio work"
              >
                View My Work
                <ArrowDown className="ml-2 sm:ml-3 w-3 h-3 sm:w-5 sm:h-5" aria-hidden="true" />
              </Button>
              <Button 
                onClick={handleGetInTouch} 
                variant="outline" 
                className="border-2 border-blue-400 text-blue-400 hover:bg-blue-400 hover:text-white px-4 sm:px-8 lg:px-10 py-3 sm:py-6 lg:py-7 text-sm sm:text-lg rounded-lg sm:rounded-2xl transition-all duration-300 shadow-lg bg-transparent touch-manipulation min-h-[44px] sm:min-h-[52px] w-full sm:w-auto"
          ...
          <div className="flex justify-center lg:justify-end order-1 lg:order-2 mb-4 sm:mb-0">
            ...
            >
              <div className="profile-glow w-48 h-48 sm:w-72 sm:h-72 md:w-80 md:h-80 lg:w-[28rem] lg:h-[28rem] xl:w-[32rem] xl:h-[32rem] rounded-full overflow-hidden animate-float shadow-2xl">
                <img 
